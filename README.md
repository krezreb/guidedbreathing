...
│   └── sounds.js            iOS-safe audio unlock; chime and phase cues
...
**The sounds follow the phase, not a timer.** Three bundled `.wav` files: a
chime at the end of the session, and a cue at each phase boundary — a low D4
when an exhale ends (breathe in) and an A4 when an inhale ends (breathe out).
The cue is played by the same loop that draws the frame, on the first frame of
the new phase, so it cannot drift away from the animation. All three buffers are
decoded from the Begin tap, because iOS will not start audio later without a
gesture. The two cue tones are generated by `scripts/make-audio-cues.py`, a
design tool rather than a build step.
...
completed state, which phase cue sounds at each boundary (and that none sound at
the start, when paused, or over the completion chime), and translation coverage
across all six languages. Animation smoothness and layout are checked by hand on a real
device.
...
- **Audio blocked or muted** (including the iOS ringer switch) — the breathing
  guide is unaffected: the phase cues are simply inaudible, and the session
